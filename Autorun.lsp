...
(defun imp-on-doc-activated (reactor arg)
  (vlr-remove reactor)
  (imp-run-startup))

(defun S::STARTUP () (imp-run-startup))

(if (not (boundp '*imp-startup-reactor*))
  (setq *imp-startup-reactor*
    (vlr-document-reactor nil '((:vlr-documentActivated . imp-on-doc-activated)))))

; Direct call as fallback if S::STARTUP doesn't fire (document already open)
(if (not (boundp '*imp-ran*))
  (progn
    (setq *imp-ran* T)
    (imp-run-startup)))
